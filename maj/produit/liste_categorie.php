...
<?
	...
	$num_produit_categorie = $_POST["num_produit_categorie"];
	$nom = trim($_POST["nom"]);
...
<head>
	...
	<!--
		...
		function annuler() {
			document.formulaire.num_produit_categorie.value = "";
			document.formulaire.submit();
		}
		
		function afficher(num) {
			//alert(num);
			document.formulaire.num_produit_categorie.value = num;
			document.formulaire.submit();
		}
		
		function initialiser() {
			find("num_parent").className = "";
			find("nom").className = "";
		}
		...
		function ajouter() {
			...
			if ( find("num_parent").value == 0) {
				find("num_parent").className = "erreur";
				val_erreur = 1;
			}
...
<body style="margin:5px 0;background:#688FD5;">
	...
	<form name="formulaire" method="POST" action="liste_categorie.php">
		...
		<input type="hidden" name="num_produit_categorie" value="<?=$num_produit_categorie?>">
										...
										<td width="*" align="left" valign="middle">
											<select name="num_parent" id="num_parent">
												...
												foreach( $liste_categorie_parent as $_categorie ) {
													...
													?>
													<option value="<?=$_categorie->num_produit_categorie?>" <? if ( $num_parent == $_categorie->num_produit_categorie ) { ?> selected <? } ?>>
								...
								<tr>
									<td width="*" height="20">
										...
										if ( $_categorie->niveau > 0 ) {
											?>
											<a href="javascript:afficher(<?=$_categorie->num_produit_categorie?>);" style="margin-left: <?=$decalage?>px;"><?=stripcslashes( utf8_encode( $_categorie->nom ) )?></a>
											<?
										}
									...
									<td width="40" align="right" valign="middle">
										...
										if ( $_categorie->niveau > 0 ) {
											?>
											<a href="javascript:afficher(<?=$_categorie->num_produit_categorie?>);"><img src="../images/icones/edit.gif" border="0" title="Modifier la cat&eacute;gorie"></a>
											<a href="javascript:supprimer(<?=$_categorie->num_produit_categorie?>);"><img src="../images/icones/delete.gif" border="0" title="Supprimer la cat&eacute;gorie"></a>
											<?
										}
										else echo "&nbsp;";
										?>
